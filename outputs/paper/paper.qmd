...
knitr::kable(sample_data,'markdown',col.names = c("Year of Death", "Cause of Death", "Age Group", "Gender", "Count"),align = 'c',
                      ...
                      caption = "Sample Table for Causes of Death")
  # Print the table

```

```{r}
#| label: fig-causes-of-death
#| fig-cap: Causes of death analysis
#| echo: false


ggplot(data = toronto_homeless_death,mapping =  aes(x = Cause_of_death, fill = Age_group)) +
...
#\| label: fig-planes #\| fig-cap: Relationship between wing length and
width #\| echo: false #\| warning: false #\| message: false

analysis_data \<- read_csv(here::here("outputs/data/analysis_data.csv"))

analysis_data \|\> ggplot(aes(x = width, y = length)) + geom_point(alpha
= 0.8) + theme_minimal() + labs(x = "Wing width (mm)", y = "Wing length
(mm)")
